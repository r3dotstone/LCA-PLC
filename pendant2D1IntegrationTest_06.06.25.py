...
# Configure Position Pins
GPIO.setup(pPin0,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
GPIO.setup(pPin1,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
GPIO.setup(pPin2,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
GPIO.setup(pPin3,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
GPIO.setup(pPin4,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
GPIO.setup(pPin5,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)

# Configure Velocity Pins
GPIO.setup(vPin0,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
GPIO.setup(vPin1,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
GPIO.setup(vPin2,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
GPIO.setup(vPin3,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
GPIO.setup(vPin4,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
GPIO.setup(vPin5,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
...
pos = [int(row[0]) for row in params]
vel = [int(row[1]) for row in params]
midP = int(params[0][2])
...
def pins2Num(pin0, pin1, pin2, pin3):
    b0 = GPIO.input(pin0)
    b1 = GPIO.input(pin1)
    b2 = GPIO.input(pin2)
    b3 = GPIO.input(pin3)
    ...
    while error == 0:        
        ...
        while start == 1:
            ...
            while ref_done == 1:
                ...
                pIndex = pins2Num(pPin0, pPin1, pPin2, pPin3)
                vIndex = pins2Num(vPin0, vPin1, vPin2, vPin3)
                pDesired = pos[pIndex]
                vDesired = vel[vIndex]
                
                if state == 0x00:
                    abs(midP-pDesired, vDesired)
                    state = 0x01
